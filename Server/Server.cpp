...
void Server::handleRecievedMessages()
{
	...
	while (true)
	{
		...
		case SIGN_UP:
			...
			break;
		case ROOM_JOIN:
			*saved = _queRcvMessages.front()->getSock();
		...
		case ROOM_CREATE:
			...
			break;
		case MOUSE_MOVE:
			handleMouseMove(*saved,"0360003300");
			break;
		default:
			handleSignOut(_queRcvMessages.front());
...
void Server::handleMouseMove(SOCKET socket, string msgToSend)
{
	Helper::sendData(socket, msgToSend);
}
