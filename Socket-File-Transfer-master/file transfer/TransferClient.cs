...
namespace file_transfer
{
    ...
    public class TransferClient
    {
        ...
        private byte[] _buffer = new byte[999999];
        ...
        {
            PacketReader pr = new PacketReader(Main.Decrypt(_buffer)); //Create our packet reader.
                ...
                case Headers.Queue:
                    {
                        ...
                        // here for the statistics אפשר לשלוח מפה את שם הקובץ והגודל לסטטיסטיקות
        ...
        private void receiveCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (found >= 4)
                {
                    if(Main.key == null)
                    {
                        if (Main.firstKey == null)
                        {
                            // now need to read the AES key
                            //We will receive our size bytes
                            _baseSocket.Receive(_buffer, 0, 4, SocketFlags.None);

                            //Get the int value.
                            int s = BitConverter.ToInt32(_buffer, 0);
                            //Main.key = new byte[s];
                            Main.firstKey = new byte[s];
                            //And attempt to read our
                            //_baseSocket.Receive(Main.key, 0, s, SocketFlags.None);
                            _baseSocket.Receive(Main.firstKey, 0, s, SocketFlags.None);

                            // now need to read the AES IV
                            _baseSocket.Receive(_buffer, 0, 4, SocketFlags.None);

                            //Get the int value.
                            s = BitConverter.ToInt32(_buffer, 0);
                            //Main.IV = new byte[s];
                            Main.firstIV = new byte[s];
                            //And attempt to read our
                            //_baseSocket.Receive(Main.IV, 0, s, SocketFlags.None);
                            _baseSocket.Receive(Main.firstIV, 0, s, SocketFlags.None);
                            ...
                            using (Aes aes = Aes.Create())
                            {
                                ...
                                //key = aes.Key;
                                //IV = aes.IV;
                                Main.secondKey = aes.Key;
                                Main.secondIV = aes.IV;
                            }
                            Send(Main.secondKey);
                            Send(Main.secondIV);
                        ...
                        if (Main.secondKey == null)
                        {
                            ...
                            int s = BitConverter.ToInt32(_buffer, 0);
                            //Main.key = new byte[s];
                            Main.secondKey = new byte[s];
                            //And attempt to read our
                            //_baseSocket.Receive(Main.key, 0, s, SocketFlags.None);
                            _baseSocket.Receive(Main.secondKey, 0, s, SocketFlags.None);
                            ...
                            s = BitConverter.ToInt32(_buffer, 0);
                            //Main.IV = new byte[s];
                            Main.secondIV = new byte[s];
                            //And attempt to read our
                            //_baseSocket.Receive(Main.IV, 0, s, SocketFlags.None);
                            _baseSocket.Receive(Main.secondIV, 0, s, SocketFlags.None);
                            System.Threading.Thread.Sleep(1000);
                        }

                        //creating the main key
                        Main.key = new byte[Main.firstKey.Length];
                        Main.IV = new byte[Main.firstIV.Length];
                        // creating the main key
                        for (int i=0;i< Main.firstKey.Length; i++)
                        {
                            if (i % 2 == 0)
                            {
                                Main.key[i] = Main.firstKey[i];
                            }
                            else
                            {
                                Main.key[i] = Main.secondKey[i];
                            }
                        }
                        // creating the main IV
                        for (int i=0;i< Main.firstIV.Length; i++)
                        {
                            if (i % 2 == 0)
                            {
                                Main.IV[i] = Main.firstIV[i];
                            }
                            else
                            {
                                Main.IV[i] = Main.secondIV[i];
                            }
                        }
                        System.Threading.Thread.Sleep(1000);
                    }
