...
public partial class Main : Form
{
    ...
    public void sendStatistic(string file)
    {
        ...
        string sendersIp = localIp; // the ip of the sender
        string reciversIp = localIp;// the ip of the reciver
        string fileType = file.Substring(file.IndexOf("."), file[file.Length]);
        string time = DateTime.Now.ToShortTimeString(); // the current time 
        string toSend = Constants.STATISTIC_MSG + sendersIp.Length + sendersIp + reciversIp.Length + reciversIp + fileType.Length + fileType + time.Length + time;
